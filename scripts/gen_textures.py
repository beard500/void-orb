...
Outputs two 16x16 PNGs into src/main/resources/assets/void_orb/textures/item/:
...
to author the original model. Run before `./gradlew build`; CI runs this
automatically via the Build workflow.
"""
import os
import struct
import zlib
...
TEX_DIR = os.path.join(ROOT, "src", "main", "resources", "assets", "void_orb", "textures", "item")
...
def make_orb() -> list[list[tuple[int, int, int, int]]]:
    ...
    # Deep purple highlights (top-left lit nebula specks)
    ...
    # Mid-purple glow scattering on the lower half
